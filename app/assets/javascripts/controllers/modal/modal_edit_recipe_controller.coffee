@cooking.controller 'ModalEditRecipeController', ( $scope, $rootScope, $modalInstance, datacontext, Notification, recipe, volumes, weights, measures, UploaderImage, FileUploader, $http ) ->
  ...
  url = 'uploads/image/avatar/1/MrouaDoniaPhoto.jpg'
  $http.get(url, {responseType: "blob"}).success( (data, status, headers, config) ->
    mimetype = data.type
    file = new File([data], "MrouaDoniaPhoto.jpg", {type:mimetype})
